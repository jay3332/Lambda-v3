...
from bot.utils.common import to_thread
from bot.utils.pillow import alpha_paste, rounded_mask

if TYPE_CHECKING:
    from discord import Member, User

    from bot import Lambda
    from bot.types.common import RGBColor, RGBAColor
    from bot.types.leveling import RankCard as RankCardPayload
...
class BaseRankCard:
    ...
    def __init__(self, *, data: RankCardPayload, user: Member | User, bot: Lambda) -> None:
    ...
    def _from_data(self, data: RankCardPayload) -> None:
        ...
        self.overlay_color: RGBAColor = *overlay_color, int(data['overlay_alpha'] * 255)
        self.overlay_border_radius: int = data['overlay_border_radius']

        avatar_border_color = _(data['avatar_border_color'])
        self.avatar_border_color: RGBAColor = *avatar_border_color, int(data['avatar_border_alpha'] * 255)
        self.avatar_border_radius: int = data['avatar_border_radius']

        progress_bar_color = _(data['progress_bar_color'])
        self.progress_bar_color: RGBAColor = *progress_bar_color, int(data['progress_bar_alpha'] * 255)
...
class RankCard(BaseRankCard):
        ...
        *,
        ...
        with Image.new('RGBA', (1250, 460)) as image:
            ...
            with Pilmoji(
                ...
                if unidecode(username) == username:
                    font = self.font
                    y_shift = 0
                else:
                    font = Font.ARIAL_UNICODE
                    y_shift = 5

                font = fonts.get('./assets/fonts/' + FONT_MAPPING[font.value], size=55)
                pilmoji.text((406, 189 - y_shift), username, self.primary_color, font)
                width, _ = pilmoji.getsize(username, font)

                # Discriminator
                font = get_font(size=50)
                text = '#' + self.user.discriminator
                pilmoji.text((409 + width, 194), text, (*self.secondary_color[:3], 190), font)
    ...
    async def render(self, *, rank: int, level: int, xp: int, max_xp: int) -> BytesIO:
        ...
        background = await self.prepare_background(background_bytes)
